...
/*
 * run a test and update the stats. The main guts of this functionality is
 * provided by invoke_test_with_timelimit This outer wrapper updates thes output
 * and statistics before and after running the test.
 */
static int run_one_test(stats_t *stats, testentry_t *test,
                        int redirect_stdouterr, int argc, char **argv) {
  int test_result;
...
static void print_targets(testentry_t tests[], int count) {
  ...
  for (i = 1; i < 5; i++) printf("%s ", strategy_name(i));
  printf("\n");
}
