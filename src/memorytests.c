...
/* performs a randomized test:
        ...
        fillRatio == when the allocated memory is >= fillRatio * totalSize, a
   block is freed; otherwise, a new block is allocated. If a block cannot be
   allocated, this is tallied and a random block is freed immediately thereafter
   in the next iteration minBlockSize, maxBlockSize == size for allocated blocks
   is picked uniformly at random between these two numbers, inclusive
        */
void do_randomized_test(int strategyToUse, int totalSize, float fillRatio,
                        int minBlockSize, int maxBlockSize, int iterations) {
  void* pointers[10000];
  ...
  if (strategyToUse > 0) lbound = ubound = strategyToUse;
  ...
  fprintf(log,
          "Running randomized tests: pool size == %d, fill ratio == %f, block "
          "size is from %d to %d, %d iterations\n",
  ...
  for (strategy = lbound; strategy <= ubound; strategy++) {
    ...
    fprintf(log, "\tTest took %.2fms.\n",
            (execend.tv_sec - execstart.tv_sec) * 1000 +
                (execend.tv_nsec - execstart.tv_nsec) / 1000000.0);
...
int do_stress_tests(int argc, char** argv) {
  ...
  do_randomized_test(strategy, 10000, 0.5, 1000, 1000,
                     10000); /* watch what happens with this test!...why? */
  ...
  for (strategy = lbound; strategy <= ubound; strategy++) {
    ...
    for (i = 0; i < 100; i++) {
      ...
      if (i > 0 && pointer != (lastPointer + 1)) {
        printf(
            "Allocation with %s was not sequential at %i; expected %p, actual "
            "%p\n",
...
int test_alloc_2(int argc, char** argv) {
  ...
  for (strategy = lbound; strategy <= ubound; strategy++) {
    ...
    if (second != (first + 10)) {
      printf(
          "Second allocation failed; allocated at incorrect offset with "
          "strategy %s",
          strategy_name(strategy));
      return 1;
    }
    ...
    if (!correctThird) {
      printf("Third allocation failed; allocated at incorrect offset with %s",
             strategy_name(strategy));
      return 1;
    }
  ...
  for (strategy = lbound; strategy <= ubound; strategy++) {
    ...
    for (i = 1; i < 100; i += 2) {
      ...
      if (i > 1 && pointer != (lastPointer + 2)) {
        printf(
            "Second allocation with %s was not sequential at %i; expected %p, "
            "actual %p\n",
            strategy_name(strategy), i, lastPointer + 1, pointer);
        return 1;
      }
...
int run_memory_tests(int argc, char** argv) {
  ...
  testentry_t tests[] = {
      {"alloc1", "suite1", test_alloc_1},    {"alloc2", "suite2", test_alloc_2},
      {"alloc3", "suite1", test_alloc_3},    {"alloc4", "suite2", test_alloc_4},
...
int main(int argc, char** argv) {
  if (argc < 2) {
    printf(
        "Usage: mem -test <test> <strategy> | mem -try <arg1> <arg2> ... \n");
    exit(-1);
